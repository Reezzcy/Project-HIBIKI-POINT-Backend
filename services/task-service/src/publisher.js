...
async function connectPublisher() {
    try {
        ...
        console.log('[Task Service] Publisher connected to Exchange');
        ...
        connection.on('close', () => {
            console.warn('[Task Service] RabbitMQ Connection closed.');
        ...
        channel.on('error', (err) => {
            console.error(
                '[Task Service] RabbitMQ Channel error:',
                err.message
            );
            channel = null; // Invalidate channel on error
        });
...
function publishEvent(routingKey, data) {
    if (channel) {
        try {
            ...
            console.log(
                `[Task Service] Published event '${routingKey}' to exchange '${exchangeName}'`
            );
        } catch (error) {
            console.error(
                `[Task Service] Failed to publish event '${routingKey}':`,
                error.message
            );
        }
    } else {
